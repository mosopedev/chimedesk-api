...
class UserService {
    public async onboard(user: string, bio: string, countryCode: string, yearsOfExperience: number, stateOrProvince: string, country: string, photo: any, resume: any): Promise<IUser> {
        try {
            ...
            logger(error)
            throw new Error(translateError(error)[0] || 'Profile setup failed. Try again')
        }
    }
