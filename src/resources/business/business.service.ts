...
class BusinessService {
    ...
    public async getBusiness(id: string): Promise<IBusiness | undefined> {
        try {
            const business: IBusiness | null = await businessModel
                .findById(id)
                .select(
                    "name uniqueName website humanOperatorPhoneNumbers email country allowedActions"
                );
    ...
    public async getBusinessKnowledgeBase(id: string): Promise<IBusiness | undefined> {
        try {
            const business: IBusiness | null = await businessModel
                .findById(id)
                .select(
    ...
    public async addAction(
        ...
        try {
            ...
            logger(error)
            throw new Error(error || "Unable to setup agent actions, please try again.");
        }
    }
    ...
    public async removeAction(businessId: string, actionId: string) {
        try {
            ...
            throw new Error(error || "Unable to remove agent actions, please try again.");
        }
    }

    public async getBusinessActions(businessId: string) {
        try {
            const actions = await businessModel.findById(businessId).select('allowedActions');
            ...
            throw new Error(error || "Unable to retrieve agent actions, please try again.");
        }
    }
}
